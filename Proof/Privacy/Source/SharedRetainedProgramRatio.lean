...
/-- The actual shared programming mass fits the real unused-label source average. -/
theorem retained_program_ratio [Fintype Block]
    ...
    (residualFits : ∀ index, sharedCircuitBucketSize index +
      Fintype.card (ResidualQueryDomain (state.fixedTranscript ++ queries)
        (SharedQueryCounts.programmedDomains (Shared.Simulator.commandRecords values)) index) ≤
          Fintype.card Block) :
    (1 - ((54690772 + (364 * (state.fixedTranscript ++ queries).length : Nat)) / (2 : ENNReal) ^ 128)) *
      ...
      ∑' labels, (PMF.uniformOfFintype (EncPRF.PermutationIndex → Block)) labels *
        (PMF.uniformOfFintype (PermutationOracle Shared.FixedKeyIndex Block)).toOuterMeasure
          {fixedOracle | RawGarblingMatches (context.gates (hidden, labels)) (Shared.expandOracle fixedOracle) ∧
            PermutationTranscriptMatches fixedOracle (state.fixedTranscript ++ queries)} := by
  ...
  rw [domains] at ratio
  have average := linked_unusedLabel_average_ge context oracle bridgeKey linked hidden anchor pointGood
